...
summary: The Spyder team hosted another Birds of a Feather session at SciPy 2024, this time on the topic of users' experiences (good and bad) with the UI/UX of scientific interfaces and IDEs, and how their developers can better serve users. Here, we share what we learned from the session, as well as a link to the full detailed community notes.
---

The Spyder team hosted a Birds of a Feather (BoF) session, [What do you want to see from your scientific IDE?](https://cfp.scipy.org/2024/talk/VWMV7B/), at [SciPy 2023](https://www.scipy2024.scipy.org/).
...
In the room, we had not only myself but also [Pierre Raybaut](https://pierreraybaut.github.io/), the original Spyder creator himself (who also gave a great [talk on Spyder's history and his current work](https://www.youtube.com/watch?v=MKPUuV1LEqk)), as well as another former Spyder developer.
...
Participants empathized that scientific coding is different from other forms of programming, and that the "Copilots of the world" may not help w/scientific aspects of coding.
Indeed, they would not necessarily be expected such as properly designing valid digital experiments, correctly implementing appropriate scientific algorithms, ensuring the analysis techniques used are valid for the data, and choosing appropriate metrics and statistics to  interpret the results.
...
Another participant averred that more so than citations for code, the most important thing AI would help with is tests that verify that the code does what it is supposed to do.
...
THey also expressed a desire for IDEs having native support for interactive map displays like in QGIS or ArcGIS rather than just static Matplolib images, which would make more more rapid prototyping.
...
Finally, a PyCharm user shared how Xarray/pandas, their extension system and data accessors were all opaque to its autocomplete, at least without further tagging or augmentation to the IDE / code.
...
Posing the question directly, we asked people to share their on a web-based application vs. a desktop application, and on a "desktop" app using a webview as its UI.
...
On a related note, an attendee shared that they support a small statics group, and their biggest challenge is helping students learn scientific computing.
...
This prompted others to related how that where they work, platforms for accessing data require a lot of authentication, and entail using JupyterHub as their default IDE/platform, with very little flexibility in the matter.
They would like a platform for other IDEs to be able to connect to a shared JupyterHub environment, which confidential is exactly what Spyder is working on for Spyder 6 and beyond.


## GPU compute integration

One fairly specific topic that got a disproportionate amount of attention was incorporating better support for GPU compute (e.g. CUDA) within IDEs and interfaces, thanks to Nvidia representatives in the room posing the initial question.
...
Another attendee has a postdoc using a bunch of A100 GPUs, and asked them whether they were getting the most performance out of it.
The postdoc's reply, which the attendee found rather telling, was that "PyTorch runs"—it was painful enough for them to just get it running. so they were not particularly interested in spending even more time optimizing it further.
...
They have students who don't know how to use the 4 CPUs in their laptop, much less the GPUs in the supercomputer center.
...
But, they said, it poses a real sociological challenge to get students used to using, much less optimizing GPUs.

Finally, it was asked what IDEs could do to make GPU workflows more convenient.
In reply, participants suggested better integration with NVSMI or an integrated dashboard of GPU usage, which would allow users to see that, for instance, a particular kernel is crunching data is too big to use the full potential of the GPU.
...
All told, there was a surprising amount of interest and discussion around this once-niche topic, signaling that GPU compute has become something much closer to a mainstream need in the scientific space.
...
A particularly salient comment was one attendees request that "We need RStudio for Python", with another replying that Spyder fills most of that gap.
...
By contrast, the most surprising was the extensive discussion about GPU compute and how IDEs and tools can help with that, which seems to indicate that IDE plugins for assisting such might be valuable.

Another area of user emphasis was the importance of interactive data exploration for a variety of formats and structures.
While a subset of such could be built into the IDE (as we have with Spyder's Variable Explorer), given the diversity of scientific data types across fields this suggests the need for a more pluggable/extensible approach to variable exploration.
...
To that end, we've developed an initial version of for Spyder 6.0 as a JupyterHub plugin, with more extensive capabilities to follow in Spyder 6.1, and could eventually be developed into such an common interface.

Thanks to everyone for attending and sharing such great ideas and insights, and we look forward to continuing to learn from them and hear more in the future.
